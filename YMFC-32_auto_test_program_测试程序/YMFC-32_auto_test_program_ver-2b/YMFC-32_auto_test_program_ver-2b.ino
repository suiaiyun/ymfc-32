...
uint8_t gyro_address = 0x68;               // MPU-6050 陀螺仪器件地址
uint8_t MS5611_address = 0x77;             // MS5611 气压计器件地址
uint8_t compass_address = 0x0D;            // QMC5883L 电子罗盘器件地址
...
void setup() {
  ...
  if (digitalRead(PB3) && digitalRead(PB4)) {
    flip32 = 1;         //Input PB3 and PB4 are high on the Flip32
  } else {
    flip32 = 0;
  }
  flip32 = 0;
  ...
  HWire.write(0x80);                                            //Set the register bits as 00000000 to activate the gyro.
...
void loop() {
  ...
  if (data == 't') {
    Serial.println(F("Reading the raw temperature data."));
    Serial.println(F("You can exit by sending a q (quit)."));
    delay(2500);
    read_gyro_values();
  }
  ...
  if (data == 'j') {
    Serial.println(F("Checking QMC5883L compass."));
    Serial.println(F("You can exit by sending a q (quit)."));
    delay(2500);
    check_compass();
  }
