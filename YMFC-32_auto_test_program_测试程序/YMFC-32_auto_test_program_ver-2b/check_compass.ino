...
void check_compass(void) {
  ...
  if (error != 0) {                                          //Print a message when the compass did not respond.
    ...
    HWire.beginTransmission(compass_address); //open communication with QMC5883
    HWire.write(0x09);
    HWire.write(0x1d);
    HWire.endTransmission();
  }
  ...
  while (data != 'q') {                                                   //Stay in this loop until the data variable data holds a q.
    ...
    HWire.beginTransmission(compass_address);
    HWire.write(0x00); //select register 0, X LSB register
    HWire.endTransmission();
...
void read_data() {
  ...
  if(compass_x > 0x7fff) compass_x -= 0xffff;
  if(compass_y > 0x7fff) compass_y -= 0xffff;
  if(compass_z > 0x7fff) compass_z -= 0xffff;
}
